(() => {
  ...
  function friendly(error){if(/permission|unauthorized/i.test(error.code||error.message))return 'Firebase access is not enabled yet. Apply the Level Pack Creator rules.';if(/operation-not-allowed/i.test(error.code||''))return 'Enable Anonymous sign-in in Firebase Authentication first.';return error.message||'Unable to connect to Firebase. Please try again.';}
  function valid(pack){
